...
This distribution results from the fact that photons with $r_\mathrm{min}>0$ values can experience
total reflection even if their angle $\theta$ is larger than the already determined maximal angle. 
This is explained by the effective angle between the photon and the cladding beeing lower then $\theta$ 
when the photon in not passing through the middle of the fiber.
...
\end{equation}
is used for the fits. A subsection of the detrmined fits for different horizontal angles is shown in \autoref{fig:simfits}.
